...
export function LabelScreen() {
  ...
  const generateLabel = async () => {
    ...
    try {
      setGenerated(await labelApi.create(item, selectedProject.name));
    } catch (error) {
  ...
  const printLabel = async () => {
    ...
    try {
      ...
      await printMaterialLabel({ ...item, projectName: selectedProject.name, labelId: generated.label.id }, qrBase64);
    } catch (error) {
  ...
  const shareLabel = async () => {
    ...
    try {
      const qrBase64 = getQrBase64();
      const shared = await shareMaterialLabel({ ...item, projectName: selectedProject.name, labelId: generated.label.id }, qrBase64);
