...
export function FeatureScreen({ type }) {
  const { openScreen, openScanner, selectedProject } = useApp();
  const baseView = config[type] || config.stock;
  const view = type === 'stock' ? { ...baseView, eyebrow: selectedProject.name.toUpperCase() } : baseView;
