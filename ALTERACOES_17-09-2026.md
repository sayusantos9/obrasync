...
## Refatoração adicional — orçamento e organização do código

- Orçamento recebeu as abas internas **Resumo** e **Itens do orçamento**.
- Criado formulário funcional para adicionar itens ao orçamento por obra, com categoria, unidade, quantidade e preço unitário.
- Itens do orçamento são persistidos em `AsyncStorage`, podem ser removidos e entram no total previsto.
- PDF do orçamento passou a incluir os itens adicionados manualmente.
- `AppNavigator` foi reduzido e a resolução de telas foi isolada em `screenRegistry.js`.
- `ProfileScreen`, `ProjectsScreen` e `ScannerScreen` foram reorganizados com nomes padronizados e componentes reutilizáveis.
- Scanner foi dividido em `ScannerHeader`, `ScannerCameraPanel` e `ScannerActions`.
- Estilos extensos foram retirados das telas principais e organizados em arquivos/componentes próprios.
