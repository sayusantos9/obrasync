...
function buildBudgetItemRows(items) {
  ...
  return `
    ...
    <table>
      <thead><tr><th>Item</th><th>Categoria</th><th>Quantidade</th><th>Preço unitário</th><th>Total</th></tr></thead>
      <tbody>${rows}</tbody>
    </table>`;
}

function buildBudgetHtml(reportData) {
  const {
    balance,
    budget,
    budgetItems = [],
  ...
  <html>
    <head>
      ...
      <style>
        ...
        h1{font-size:24px;margin:24px 0 5px}h2{font-size:18px;margin:26px 0 8px}.muted{color:#756D7A;font-size:12px}
    ...
    <body>
      ...
      <div class="summary">
        <div class="box"><div class="label">Previsto</div><div class="value">${formatCurrency(plannedTotal)}</div></div>
      ...
      ${buildBudgetItemRows(budgetItems)}
