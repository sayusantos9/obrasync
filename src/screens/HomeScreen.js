...
const ProgressFill = styled.View`height: 100%; width: 68%; background-color: ${({ theme }) => theme.colors.accent}; border-radius: 5px;`;
...
export function HomeScreen() {
  const { openScreen, openScanner, selectTab, isOffline, pendingRequests, sync, isSyncing } = useApp();
  const { data, isLoading } = useDashboard();
  return <Screen><Scroll><AppHeader /><Content><SearchBar />
    <Hero source={HERO_IMAGE}><HeroShade><Status><Dot /><StatusText>EM ANDAMENTO</StatusText></Status><HeroTitle>Sua obra em dia</HeroTitle><HeroText>Residencial Aurora • Bloco A</HeroText><ProgressRow><Progress>68%</Progress><ProgressText>do cronograma{`\n`}concluído</ProgressText></ProgressRow><ProgressTrack><ProgressFill /></ProgressTrack><HeroButton onPress={() => selectTab(TABS.COSTS)}><HeroButtonText>Ver detalhes</HeroButtonText><Ionicons name="arrow-forward" size={18} color="white" /></HeroButton></HeroShade></Hero>
    ...
    <SyncCard onPress={sync}><Ionicons name={isOffline ? 'cloud-offline' : 'cloud-done'} size={30} color={isOffline ? '#B5473E' : '#5A1B73'} /><SyncCopy><SyncTitle>{isOffline ? 'Modo offline' : isSyncing ? 'Sincronizando...' : 'Sincronizado'}</SyncTitle><SyncText>{pendingRequests.length ? `${pendingRequests.length} registro(s) pendente(s)` : 'Dados atualizados recentemente'}</SyncText></SyncCopy><Ionicons name="chevron-forward" size={20} color="#5A1B73" /></SyncCard>
