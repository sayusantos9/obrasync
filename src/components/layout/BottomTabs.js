...
const IconShell = styled.View`width: 34px; height: 30px; border-radius: 11px; align-items: center; justify-content: center; background-color: ${({ active, theme }) => active ? theme.colors.accentSoft : 'transparent'};`;
const tabs = [
  ...
  { id: TABS.MORE, label: 'Mais', icon: 'ellipsis-horizontal-circle' },
];

export function BottomTabs() {
  const { activeTab, selectTab } = useApp();
  return <Bar>{tabs.map((tab) => { const active = tab.id === activeTab; return <Item key={tab.id} onPress={() => selectTab(tab.id)}><IconShell active={active}><Ionicons name={active ? tab.icon : `${tab.icon}-outline`} size={22} color={active ? '#E0A526' : '#756D7A'} /></IconShell><Label active={active}>{tab.label}</Label></Item>; })}</Bar>;
}
